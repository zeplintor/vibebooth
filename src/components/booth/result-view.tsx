...
export function ResultView({ photos, backgroundUrl, onRetake, roomId }: ResultViewProps) {
  ...
  const shareUrl = typeof window !== 'undefined'
    ...
    : ''
  const shareText = 'On a fait un photobooth en ligne 📸✨'
  ...
  return (
    <div className="flex flex-col items-center w-full pb-8 pt-2 px-4">

      {/* Title */}
      <h2 className="font-[family-name:var(--font-hand)] text-3xl font-bold text-vb-ink mb-4">
        Votre photo strip 🎉
      </h2>

      {/* Strip image — constrained so it fits any screen */}
      <div
        className="card-sketch overflow-hidden"
        style={{ rotate: '-1.5deg', maxWidth: 'min(320px, 80vw)' }}
      >
        <canvas
          ref={canvasRef}
      ...
      <div className="flex flex-col gap-3 w-full max-w-xs mt-6">
        {canNativeShare ? (
          <button
            ...
            className="btn-sketch bg-vb-pink text-white w-full flex items-center justify-center gap-2 disabled:opacity-50"
          >
        ...
        {/* Always show download separately if native share exists */}
        {canNativeShare && (
          <button
            onClick={handleDownload}
            disabled={!downloadUrl}
            className="btn-sketch bg-white text-vb-ink w-full flex items-center justify-center gap-2 disabled:opacity-50"
          >
        ...
        <div className="flex items-center justify-center gap-3 mt-1">
          <a
            ...
            className="share-btn share-btn-whatsapp w-11 h-11"
            title="WhatsApp"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="share-btn share-btn-x w-11 h-11"
            title="X / Twitter"
          >
          ...
          <button onClick={handleCopyLink} className="share-btn w-11 h-11" title="Copier le lien">
            {copied ? (
        ...
        <button
          onClick={onRetake}
          className="mt-2 font-[family-name:var(--font-hand)] text-base text-vb-ink/50 hover:text-vb-purple transition-colors underline decoration-dotted"
        >
...
function drawSketchyRect(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number) {
