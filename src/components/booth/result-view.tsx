...
export function ResultView({ photos, backgroundUrl, onRetake, roomId }: ResultViewProps) {
  ...
  return (
    <div className="flex flex-col items-center h-full min-h-0">
      ...
      {/* Actions row */}
      <div className="flex gap-2 shrink-0 mt-2 flex-wrap justify-center">
        <button onClick={onRetake} className="btn-sketch bg-white text-vb-ink !py-1.5 !px-3 !text-xs !border-2 !shadow-[3px_3px_0_var(--vb-border)]">
          Retake
        </button>
        <button onClick={handleDownload} disabled={!downloadUrl} className="btn-sketch bg-vb-pink text-white !py-1.5 !px-3 !text-xs !border-2 !shadow-[3px_3px_0_var(--vb-border)]">
          Download
        </button>
      </div>

      {/* Share row */}
      <div className="flex gap-2 shrink-0 mt-2 flex-wrap justify-center items-center">
        <span className="font-[family-name:var(--font-hand)] text-gray-400 text-sm">Share:</span>

        {/* Native share (mobile) */}
        {canNativeShare && (
          <button onClick={handleNativeShare} className="share-btn" title="Share">
